...
data_finetune_ae = pd.read_csv(url, sep=';')

labels2 = data_finetune_ae['Modell']
acc_asc = data_finetune_ae['accuracy']
time_asc = data_finetune_ae['trainingtime in s'] / 60
x_max = time_asc.max() + 1
x_min = time_asc.min()

plt.scatter(time_asc, acc_asc, label='GLUE', alpha=0.7, c='#8aab33')

plt.xlabel('Trainingszeit [min]')
plt.ylabel('Performance [accuracy]')
# plt.title('finetuning Effizienz - ASC')
plt.xlim(x_max, 0) # erst max weil weniger Zeit besser ist
# plt.ylim(y_min - 0.2, y_max + 0.2)
...
plt.show()
# plt.savefig('/home/johannes/Dropbox/Praktisches Seminar/Darstellungen/finetuning_effizienz_ae_acc.png', dpi=300)
