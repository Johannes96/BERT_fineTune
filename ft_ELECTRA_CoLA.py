...
# source: https://github.com/richarddwang/electra_pytorch/blob/master/finetune.py
# %%
import os, sys
...
if c.size == 'small':
    c.lr = 3e-4; c.layer_lr_decay = 0.8; c.max_length = 128
elif c.size == 'base':
    c.lr = 1e-4; c.layer_lr_decay = 0.8; c.max_length = 512
elif c.size == 'large':
    c.lr = 5e-5; c.layer_lr_decay = 0.9; c.max_length = 512
else:
...
if c.logger == 'neptune':
    ...
    class LightWandbCallback(Callback):
        ...
        def after_epoch(self):
            ...
            wandb.log({n: s for n, s in zip(self.recorder.metric_names, self.recorder.log) if
                       n not in ['train_loss', 'epoch', 'time']})
...
def get_glue_learner(task, run_name=None, inference=False):
    ...
    if run_name and not inference:
        if c.logger == 'neptune':
            ...
            wandb_run = wandb.init(name=run_name, project='electra_glue', config={'task': task, **c, **hparam_update},
                                   reinit=True)
